...
(function (root) {
  ...
  function onAuth(user) {
    ...
    BE.profile.mine().then(function (p) {
      ...
      // If they signed in via Google and have no real username yet, prompt for one.
      // "Real" = not blank AND not their Google display name (which has spaces).
  ...
  // Google display names typically have spaces, capitals, or match auth metadata.
  function isAutoGeneratedUsername(username, user) {
    if (!username) return true;
    if (/\s/.test(username)) return true;                      // any spaces = not a real username
    if (/[A-Z]/.test(username) && username.length > 6) return true; // CamelCase = display name
  ...
  // Convert "Peter Broas" → "peterbroas", strip illegal chars, lowercase
  ...
  function promptUsername(suggested) {
    ...
    setTimeout(function () {
      ...
      // Add a hint line under the title
      ...
      // Try to find an available variation of the suggested name automatically
  ...
  // Try base name, then base1, base2, ... until one is available (or give up at 9).
